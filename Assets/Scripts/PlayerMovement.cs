...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Movement()
    {
        ...
        if (Gamepad.current != null && (moveInput.sqrMagnitude >= 0f && moveInput.sqrMagnitude <= 1f)) // Rotación con el joystick derecho
        {
    ...
    private void Rotate()
    {
        if (rotateInput.sqrMagnitude >= 0f && rotateInput.sqrMagnitude <= 1f) // Rotación con el joystick derecho
        {
            Vector3 direction = new Vector3(rotateInput.x, 0f, rotateInput.y);
            RotateTowardsDirection(direction);
            //Debug.Log("Mueve");
        }
        else // if (Mouse.current != null) // Verificar si hay un mouse conectado
        {
